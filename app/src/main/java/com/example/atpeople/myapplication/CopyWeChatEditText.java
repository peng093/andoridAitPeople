...
public class CopyWeChatEditText extends AppCompatEditText {
    ...
    public void addSpan(String text) {
        SpannableString sps=MainActivity.getSpan(text);
        getText().insert(getSelectionEnd(),sps);
        DataSpan myTextSpan = new DataSpan();
        getText().setSpan(myTextSpan, getSelectionEnd() - sps.length(), getSelectionEnd(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
